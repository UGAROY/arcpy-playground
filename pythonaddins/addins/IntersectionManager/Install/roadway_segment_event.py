import arcpy
from tss_util import transform_dataset_keep_fields, populate_auto_increment_id, alter_field_name
...
class RoadwaySegmentEvent:
    ...
    def populate_from_to_measure_fields(self):
        ...
        with arcpy.da.UpdateCursor(self.roadway_segment_event, ("SHAPE@", self.roadway_segment_from_meas_field, self.roadway_segment_to_meas_field)) as cursor:
            for row in cursor:
                shape = row[0]
                row[1] = round(min(shape.firstPoint.M, shape.lastPoint.M), self.measure_scale)
                row[2] = round(max(shape.firstPoint.M, shape.lastPoint.M), self.measure_scale)
